...
static inline bool uart_send_str(const char *str, iface_ctx_t uart_hdl)
{
    size_t str_size = strlen(str);
    buffer_t buffer = {(uint8_t*)str, str_size};

    return str_size == uart_hdl->uart_iface->write(uart_hdl, &buffer);  
}

static inline bool send_at_cmd_execute(const char *cmd, iface_ctx_t uart_hdl)
{
    char cmd_out[AT_CMD_SIZE] = {'\0'};

    (void)strcat(at_command_execute(cmd, cmd_out), ESP8266_AT_SERIAL_IFACE_LINE_ENDING);

    return uart_send_str(cmd_out, uart_hdl);
}

static inline bool send_at_cmd_inquiry(const char *cmd, iface_ctx_t uart_hdl)
{
    char cmd_out[AT_CMD_SIZE] = {'\0'};
...
static inline bool send_at_cmd_set(const char *cmd, char *data, iface_ctx_t uart_hdl)
{
    char cmd_out[AT_CMD_SIZE] = {'\0'};
...
static void esp8266_fsm_at_ping(void *event_data)
{
    ...
    iface_ctx_t uart_hdl = ((esp8266_fsm_event_data_t*)event_data)->base.dev_init_data->uart_hdl;

    if (!uart_send_str(AT_CMD ESP8266_AT_SERIAL_IFACE_LINE_ENDING, uart_hdl))
    {
        // TODO: error handling
    }
}
...
static void esp8266_fsm_at_reset(void *event_data)
{
    ...
    if (!send_at_cmd_execute(ESP8266_AT_CMD_RESET, uart_hdl))
    {
...
static void esp8266_fsm_at_get_cwmode(void *event_data)
{
    ...
    if (!send_at_cmd_inquiry(ESP8266_AT_CMD_CWMODE, uart_hdl))
    {
        // TODO: error handling, callback usage
    }
}
...
static void esp8266_fsm_at_set_cwmode(void *event_data)
{
    ...
    if (!send_at_cmd_set(ESP8266_AT_CMD_CWMODE, cmd_data, uart_hdl))
    {
        // TODO: error handling, callback usage
    }
}
