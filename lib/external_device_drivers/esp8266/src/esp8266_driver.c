...
esp8266_device_handle_t create_esp8266_device(esp8266_device_init_data_t *init_data)
{
    ...
    if (init_data)
    {
        if (init_data->uart_hdl && init_data->time_iface_hdl)
        {
            if (init_data->uart_hdl->uart_iface && init_data->uart_hdl->time_iface)
            {
                ...
                if (handle)
                {
                    ...
                    iface_ctx_t uart_hdl = handle->init_data.uart_hdl;

                    uart_hdl->uart_iface->begin(uart_hdl, ESP8266_AT_SERIAL_IFACE_SPEED);
                    uart_hdl->uart_iface->flush(uart_hdl);
...
void esp8266_tick(esp8266_device_handle_t handle)
{
    ...
    const uart_iface_t *uart = uart_hdl->uart_iface;
    
    uint8_t *uart_buffer_end = &(handle->uart_buffer[sizeof(handle->uart_buffer)-1]);

    size_t available_bytes_for_read = uart->available_for_read(uart_hdl);
    if (available_bytes_for_read)
    {
        ...
        if (free_buffer_bytes)
        {
            ...
            (void)uart->read(uart_hdl, &read_buf);
        }
        else
        {
...
void destroy_esp8266_device(esp8266_device_handle_t handle)
{
    ...
    uart_hdl->uart_iface->end(uart_hdl);
